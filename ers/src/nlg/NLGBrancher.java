	...
	*/
	...
	private HashMap getGraph(HashSet<String> set, LinkedTreeMap mgraph,DefaultDirectedGraph<String, DefaultEdge> graph) {
		...
		while(itn.hasNext()){
			...
			if(tedges.containsKey(map.get("toID"))){
				setx= tedges.get(map.get("toID"));
			}
			setx.add(map);
			tedges.put((String)map.get("toID"), setx);
		}
			...
			for(DefaultEdge edge:edgs){
				...
				if(set.containsAll(prems)){
					...
					for(LinkedTreeMap mtree:tree){
						if(((String)mtree.get("fromID")).equals(prond)){
						 edges.add(mtree);
						}
					}
					//to pro node
					...
					for(LinkedTreeMap mtree:tree){
						if(prems.contains((String)mtree.get("fromID"))){
							edges.add(mtree);
						}
					}
