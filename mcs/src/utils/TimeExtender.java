...
public class TimeExtender extends TimeHelper{
	...
	public String getDateFromMoira(String text){
		long time=Long.parseLong(text);
		MutableDateTime dNow = new MutableDateTime(time,DateTimeZone.UTC);
		 DateTimeFormatter ft =  DateTimeFormat.forPattern("dd-MM-yyyy hh:mm:ss");
		   return ft.print(dNow); 
	}
	
	public String getDateFromMoiraCISpace(long time){
	...
	public DateTime parseToDateCIS(String text){
		//no matter what time zone they are checked for comparison
		DateTimeFormatter formatter = DateTimeFormat.forPattern("yyyy/MM/dd HH:mm:ss");
		DateTime dt = formatter.parseDateTime(text); 
		return dt;
	}
	...
	public String parseAndTurn(String text){
	    DateTime dt = XML_DATE_TIME_FORMAT.parseDateTime(text);
	    DateTimeFormatter ft =  DateTimeFormat.forPattern("dd-MM-yyyy hh:mm:ss");
		return ft.print(dt);
	   
	}
	public String nowUnix() {
